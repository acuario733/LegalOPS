<?php declare(strict_types=1); $estados = ['programada','realizada','cancelada']; $modalidades = ['presencial','virtual','mixta','telefonica','otra']; ?>
...
<div class="row g-4">
    <div class="col-xl-5">
        ...
        </div><div class="card-footer d-flex gap-2 flex-wrap"><a class="btn btn-outline-secondary" href="/audiencias">Volver</a><button class="btn btn-outline-success" data-action="/audiencias/<?= (int) $audiencia['id'] ?>/resultado" data-prompt="Resultado de la audiencia" data-prompt-field="resultado">Registrar resultado</button></div></div>
    </div>
    <div class="col-xl-7">
        <div class="card"><div class="card-header"><h2 class="card-title">Editar audiencia</h2></div><div class="card-body">
            <form action="/audiencias/<?= (int) $audiencia['id'] ?>" method="post" data-ajax-form>
                ...
                <div class="mb-2"><label class="form-label">Caso</label><select class="form-select" name="caso_id" required><?php foreach ($casos as $caso): ?><option value="<?= (int) $caso['id'] ?>" <?= (int) $audiencia['caso_id'] === (int) $caso['id'] ? 'selected' : '' ?>><?= e($caso['titulo']) ?></option><?php endforeach; ?></select></div>
                ...
                <div class="row g-2 mt-1"><div class="col-md-6"><label class="form-label">Modalidad</label><select class="form-select" name="modalidad"><?php foreach ($modalidades as $modalidad): ?><option value="<?= e($modalidad) ?>" <?= $audiencia['modalidad'] === $modalidad ? 'selected' : '' ?>><?= e($modalidad) ?></option><?php endforeach; ?></select></div><div class="col-md-6"><label class="form-label">Responsable</label><select class="form-select" name="responsable_usuario_id"><option value="">Sin asignar</option><?php foreach ($usuarios as $usuario): ?><option value="<?= (int) $usuario['id'] ?>" <?= (int) ($audiencia['responsable_usuario_id'] ?? 0) === (int) $usuario['id'] ? 'selected' : '' ?>><?= e($usuario['nombre']) ?></option><?php endforeach; ?></select></div></div>
                <div class="mb-2 mt-2"><label class="form-label">Despacho</label><input class="form-control" name="despacho" value="<?= e($audiencia['despacho'] ?? '') ?>"></div>
                <div class="mb-2"><label class="form-label">Lugar</label><input class="form-control" name="lugar" value="<?= e($audiencia['lugar'] ?? '') ?>"></div>
