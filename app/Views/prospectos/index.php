    ...
    <div class="col-xl-4">
        <div class="card"><div class="card-header"><h2 class="card-title">Nuevo prospecto</h2></div><div class="card-body">
            <form action="/prospectos" method="post" data-ajax-form>
                ...
                <div class="row g-2"><div class="col-sm-5"><label class="form-label">Tipo doc.</label><select class="form-select" name="tipo_documento"><option value="">Seleccione</option><?php foreach ($catalogos['tipo_documento'] as $item): ?><option value="<?= e($item['codigo']) ?>"><?= e($item['codigo']) ?></option><?php endforeach; ?></select></div><div class="col-sm-7"><label class="form-label">Documento</label><input class="form-control" name="numero_documento"></div></div>
                ...
                <div class="row g-2"><div class="col-sm-6"><label class="form-label">Fuente</label><select class="form-select" name="fuente"><option value="">Seleccione</option><?php foreach ($catalogos['origen_fuente'] as $item): ?><option value="<?= e($item['codigo']) ?>"><?= e($item['etiqueta']) ?></option><?php endforeach; ?></select></div><div class="col-sm-6"><label class="form-label">Valor estimado</label><input class="form-control" name="valor_estimado" type="number" step="0.01" min="0"></div></div>
