...
<div class="row g-4">
    <div class="col-xl-5">
        ...
        </div><div class="card-footer"><a class="btn btn-outline-secondary" href="/tareas">Volver</a></div></div>
        <div class="card mt-3"><div class="card-header"><h2 class="card-title">Gestion</h2></div><div class="card-body">
            ...
            <form action="/tareas/<?= (int) $tarea['id'] ?>/reasignar" method="post" data-ajax-form><label class="form-label">Reasignar</label><div class="input-group"><select class="form-select" name="responsable_usuario_id" required><option value="">Seleccione</option><?php foreach ($usuarios as $usuario): ?><option value="<?= (int) $usuario['id'] ?>" <?= (int) ($tarea['responsable_usuario_id'] ?? 0) === (int) $usuario['id'] ? 'selected' : '' ?>><?= e($usuario['nombre']) ?></option><?php endforeach; ?></select><button class="btn btn-outline-primary">Asignar</button></div></form>
        </div></div>
    </div>
    <div class="col-xl-7">
        <div class="card"><div class="card-header"><h2 class="card-title">Editar tarea</h2></div><div class="card-body">
            <form action="/tareas/<?= (int) $tarea['id'] ?>" method="post" data-ajax-form>
                ...
                <div class="row g-2"><div class="col-md-6"><label class="form-label">Caso</label><select class="form-select" name="caso_id"><option value="">Sin caso</option><?php foreach ($casos as $caso): ?><option value="<?= (int) $caso['id'] ?>" <?= (int) ($tarea['caso_id'] ?? 0) === (int) $caso['id'] ? 'selected' : '' ?>><?= e($caso['titulo']) ?></option><?php endforeach; ?></select></div><div class="col-md-6"><label class="form-label">Termino</label><select class="form-select" name="termino_id"><option value="">Sin termino</option><?php foreach ($terminos as $termino): ?><option value="<?= (int) $termino['id'] ?>" <?= (int) ($tarea['termino_id'] ?? 0) === (int) $termino['id'] ? 'selected' : '' ?>><?= e($termino['titulo']) ?> (<?= e($termino['fecha_vencimiento']) ?>)</option><?php endforeach; ?></select></div></div>
