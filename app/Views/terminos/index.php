    ...
    <div class="col-xl-4">
        <div class="card"><div class="card-header"><h2 class="card-title">Nuevo termino</h2></div><div class="card-body">
            <form action="/terminos" method="post" data-ajax-form>
                <div class="mb-2"><label class="form-label">Caso</label><select class="form-select" name="caso_id"><option value="">Seleccione</option><?php foreach ($casos as $caso): ?><option value="<?= (int) $caso['id'] ?>"><?= e($caso['titulo']) ?></option><?php endforeach; ?></select></div>
                <div class="mb-2"><label class="form-label">Tarea vinculada</label><select class="form-select" name="tarea_id"><option value="">Sin tarea</option><?php foreach ($tareas as $tarea): ?><option value="<?= (int) $tarea['id'] ?>"><?= e($tarea['titulo']) ?></option><?php endforeach; ?></select></div>
